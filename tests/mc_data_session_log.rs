...
fn latest_session_file_picks_most_recent_match() {
    ...
    fs::write(
        histories.join("2026-05-23-17-a.md"),
        "---\nworkspace_id: target\n---\n\n## 17:30 PT — boyan\nold\n",
    )
    .unwrap();
    std::thread::sleep(std::time::Duration::from_millis(20));
    fs::write(
        histories.join("2026-05-23-18-b.md"),
        "---\nworkspace_id: target\n---\n\n## 18:00 PT — boyan\nnew\n",
    )
    .unwrap();
    fs::write(
        histories.join("2026-05-23-19-c.md"),
        "---\nworkspace_id: other\n---\n\n## 19:00 PT — boyan\nunrelated\n",
    )
    .unwrap();
    ...
    let picked = session_log::latest_session_file_for_workspace_in_dir(&histories, "target", &ctx)
        .unwrap()
...
fn latest_session_file_returns_none_when_dir_missing() {
    let tmp = tempfile::tempdir().unwrap();
    let ctx = WorkspaceContext::default();
    let r = session_log::latest_session_file_for_workspace_in_dir(
