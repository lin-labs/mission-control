...
pub struct WorkspaceState {
    ...
    /// Repo-local Beads issues for the workspace cwd, when a `.beads/` repo is
    /// discoverable.
    pub beads: Option<crate::mc_data::beads::BeadsView>,
...
pub struct RefreshSnapshot {
    ...
    pub beads_by_ws_id: HashMap<String, crate::mc_data::beads::BeadsView>,
...
pub async fn gather_refresh_snapshot(
    ...
    let workspaces_for_beads = workspaces.clone();
    let workspaces_for_surface_intents = workspaces.clone();
    let surfaces_map_for_surface_intents = surfaces_map.clone();
    ...
    let beads_task = tokio::task::spawn_blocking(move || {
        let mut map = HashMap::new();
        for ws in workspaces_for_beads {
            if let Some(view) = crate::mc_data::beads::load_for_cwd(ws.current_directory.as_deref())
            {
                map.insert(ws.uuid, view);
            }
        }
        map
    });
    let surface_intents_task = tokio::task::spawn_blocking(move || {
        let mut out: HashMap<
            String,
            HashMap<String, crate::mc_data::session_log::ConversationIntent>,
        > = HashMap::new();
        let host = hostname_short();
        for ws in workspaces_for_surface_intents {
            let surfaces = surfaces_map_for_surface_intents
                .get(&ws.ref_id)
                .cloned()
                .unwrap_or_default();
            ...
            for (surface_idx, surface) in surfaces.iter().enumerate() {
                ...
                let same_agent_index = surfaces[..surface_idx]
                    ...
                    .count();
                let intent = crate::mc_data::session_log::resolve_session_log_for_surface(
                    &ws.uuid,
                    &surface.ref_id,
                    &ctx,
                    Some(eff.label()),
                ...
                .ok()
                .flatten()
                .and_then(|path| read_intent_from_session_path(&path));
                if let Some(intent) = intent {
                    by_surface.insert(surface.ref_id.clone(), intent);
                }
            }
    ...
    let beads_by_ws_id = beads_task.await.unwrap_or_default();
    let surface_intents_by_ws_id = surface_intents_task.await.unwrap_or_default();
    ...
    pub async fn apply_refresh_snapshot(
        ...
        self.workspaces = workspaces
            ...
            .map(|ws| {
                ...
                let beads = beads_by_ws_id.remove(&ws.uuid);
        ...
        for ws_state in self.workspaces.iter_mut() {
            ...
            // Replace the legacy goals/progress section with Beads when the
            // workspace cwd resolves to a repo-local `.beads/`. For non-Beads
            // workspaces, keep the legacy goal-row decoration path as fallback.
            let goals_section_existing = doc
                .section(crate::mc_data::trajectory::SECTION_GOALS)
                .map(|s| s.items.clone())
                .unwrap_or_default();
            let mut goals_mutated = false;
            let (goals_unchanged, goals_items_opt) = if let Some(beads) = ws_state.beads.as_ref() {
                let beads_items = beads_items_for_view(beads);
...
fn beads_items_for_view(
    view: &crate::mc_data::beads::BeadsView,
    ...
    if view.issues.is_empty() {
        ...
        let text = match view.source {
            crate::mc_data::beads::BeadsSource::Unavailable => {
                format!("Beads unavailable in {repo} (bd list failed; no issues export)")
            }
            _ => format!("No active beads in {repo}"),
        };
...
";
    ...
    fn live_beads_rows_are_read_only_in_detail_editor() {
        ...
        app.workspaces[0].beads = Some(crate::mc_data::beads::BeadsView {
            repo_path: std::path::PathBuf::from("/tmp/repo"),
            source: crate::mc_data::beads::BeadsSource::BdList,
            issues: vec![],
        });
